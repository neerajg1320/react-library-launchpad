...
function isDate(val) {
  ...
  return dateFns.isDate(val);
}
function valToString(val, format) {
  if (isDate(val)) {
    return dateFns.format(val, format ? format : indiaDateFormat);
  }
...
var TableCore = function TableCore() {
  ...
  var pluginHooks = React$1.useMemo(function () {
    ...
    if (featureColumnFilter) {
      hooks.push(reactTable.useFilters);
    }
    if (featureSorting) {
      hooks.push(reactTable.useSortBy);
    }
    if (featurePagination) {
      hooks.push(reactTable.usePagination);
    }
    if (featureSelection) {
      hooks.push(reactTable.useRowSelect);
    }
    if (layoutFixed) {
      hooks.push(reactTable.useBlockLayout);
    }
    if (layoutResize) {
      hooks.push(reactTable.useResizeColumns);
    }
  ...
  var tableInstance = reactTable.useTable.apply(void 0, [{
    columns: columns,
    data: data,
...
var GlobalFilter = function GlobalFilter(_ref) {
  ...
  var _useState = React$1.useState(globalFilter || ""),
    ...
    value = _useState2[0],
    setValue = _useState2[1];
  var _onChange = reactTable.useAsyncDebounce(function (val) {
...
exports.FeatureTable = TableWrapper;
